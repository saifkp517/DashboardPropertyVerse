...
export default function SideNav({ sendChangedComponent }: any) {
    ...
    const [activeComponent, setActiveComponent] = useState(() => {
        const savedActiveComp = localStorage.getItem('activeComp');
        return savedActiveComp ? savedActiveComp : 'properties';
    });
    const [isOpen, setIsOpen] = useState(true);

    useEffect(() => {
        localStorage.setItem('activeComp', activeComponent);
    })
                        ...
                        </li>
                        {
                            user?.role === "USER" && (
                                <>
                                    ...
                                    <li>
                                        <button onClick={() => signOut()} className={` ${activeComponent === 'logout' ? 'bg-gray-700 border border-blue-400' : ''} flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group`}>
